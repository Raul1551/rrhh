...
(function ($) {
    ...
    for (var i = 0; i < stringTypes.length; i++) {
      ...
      if (string) {
        if (normalize) {
            ...
            searchString=normalizeToBase(searchString);
        }
  ...
  Selectpicker.DEFAULTS = {
    ...
    noneResultsText: 'Ningun resultado {0}',
        ...
        if (that.activeIndex !== undefined) {
          ...
          if (init) {
            if (that.activeIndex !== that.selectedIndex) {
              if(active == undefined){
                  //no hacer nada
                  //Esto era un bug que se quedaba colgado porque no encatraba el active que es el marcado en el desplegable
              }
              else{
